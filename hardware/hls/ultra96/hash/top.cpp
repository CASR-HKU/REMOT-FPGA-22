...
#pragma HLS INLINE OFF
	...
	static key_type hashA_key[N_AU][T_SIZE];
	static value_type hashA_value[N_AU][T_SIZE];
#pragma HLS ARRAY_PARTITION variable=hashA_key dim=1 complete
#pragma HLS ARRAY_PARTITION variable=hashA_value dim=1 complete
...
DO_PRAGMA( HLS ARRAY_PARTITION variable=fifo_pointer dim=0 complete)
	...
	init_hash_array<Number>(init_n, init_hash, hashA_key, hashA_value);
	init_efifo<Number>(init_n, init_fifo, event_fifo, in_fifo_depth, fifo_pointer);

	au_hash<Number>(in_data, hashA_key, hashA_value, event_fifo, fifo_pointer, status, N);

	return_status<Number>(status_out, status);
	return_fifo<Number>(return_n, event_fifo, out_fifo);
	return_hash<Number>(return_n, hashA_key, hashA_value, out_hash);
...
DO_PRAGMA(HLS INTERFACE m_axi port=out_hash bundle=gmem0 depth=T_SIZE*T_WIDTH)
DO_PRAGMA(HLS INTERFACE m_axi port=init_hash bundle=gmem1 depth=T_SIZE*T_WIDTH)
